...
- **문제:** 기존 `StringRedisTemplate`(Lettuce) 기반 락 사용 시, 애플리케이션 레벨에서 락 소유권 확인(GET)과 해제(DEL)를 두 번의 개별 명령으로 순차 실행하는 구조였음. 그 사이 TTL이 만료되고 타 스레드가 새 락을 잡으면 **타인의 락을 오삭제할 수 있는 Race Condition**을 테스트 코드로 발견.
- **해결:**
  - **Redisson 도입 (원자성 보장 + 부가 기능 확보):** GET-then-DEL 방식의 근본 문제는 Lua 스크립트로 두 명령을 하나의 원자적 연산으로 묶으면 Lettuce 환경에서도 해결 가능하지만, 이를 직접 작성·관리하는 대신, 해당 로직과 watchdog(자동 TTL 연장), 재진입 락 등의 기능을 라이브러리 레벨에서 제공하는 Redisson으로 마이그레이션. 중간에 다른 스레드가 개입할 여지를 원천 차단하면서 락 관리 로직의 유지보수 부담도 줄임.
...
- **검증 결과:** 로직이 재개된 유저 1이 자신의 락이 만료되었음을 인지하지 못하고 **유저 2가 보유한 신규 락을 삭제**하는 현상을 확인. 근본 원인은 GET-then-DEL을 애플리케이션 레벨의 두 명령으로 나눠 처리한 구조에 있으며, Lua 스크립트로 두 명령을 하나의 원자적 연산으로 묶으면 Lettuce 환경에서도 해결 가능함. 다만 이를 직접 작성·관리하는 대신, 해당 로직을 라이브러리 레벨에서 제공하는 `Redisson`으로 도입.
